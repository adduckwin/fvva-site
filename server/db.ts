...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    } else if (user.email && ENV.adminEmails.includes(user.email.toLowerCase())) {
      values.role = 'admin';
      updateSet.role = 'admin';
    }
...
export async function getAllArticles(publishedOnly = true) {
  const db = await getDb();
  if (!db) return [];
  const query = db.select().from(blogArticles).orderBy(desc(blogArticles.createdAt));
  if (publishedOnly) {
    return query.where(eq(blogArticles.published, true));
  }
  return query;
}
...
export async function createArticle(article: InsertBlogArticle) {
  const db = await getDb();
...
export async function updateArticle(id: number, data: Partial<InsertBlogArticle>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
